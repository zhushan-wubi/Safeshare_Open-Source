...
def create_secret():
    try:
        ...
        passcode_hash = hash_password(passcode) if passcode else ""

        secret_id = str(uuid.uuid4())[:8]
        secrets[secret_id] = {
            'secret': encrypted_secret,  # 密文存储
            'passcode': passcode_hash,  # 哈希密码
            'type': secret_type,
...
def get_secret(secret_id):
    try:
        ...
        stored_pass_hash = secret.get('passcode', '')
        if secret_type == 'double':
            ...
            if stored_pass_hash and not verify_password(passcode, stored_pass_hash):
                return jsonify({'success': False, 'error': '密码错误', 'error_type': 'pass_error'}), 401


        secret_content = decrypt_secret(secret['secret'])
    ...
    try:
        ...
        for secret_id, secret in secrets.items():
            if secret.get('type') == 'tree' and not secret.get('is_deleted', False):
                # 树洞内容解密
                try:
                    content = decrypt_secret(secret['secret'])
                except:
...
def ai_reply():
    try:
        ...
        base_replies = {
            "happy": ["听你开心，我也觉得今天的晚风会软一些 🌼","真好呀，能感受到你的快乐在发光 ✨","把这份开心好好收藏，慢慢回味～ 🎈"],
            "sad": ["像下雨天坐在窗边。雨会停，但不用现在停 🤍","难过的时候不用假装坚强，我陪着你 💛","你的情绪值得被看见，慢慢来，不着急 🕯️"],
            "angry": ["像石头砸进水面。愤怒的波纹会慢慢散去，但那一刻的真实，我看见了 🔥","生气是正常的，不用压抑自己的感受 🗣️","等情绪稍微平复一点，我们慢慢说 💨"],
            "anxious": ["慢慢走，我在这。快慢都没关系 🌿","不用逼自己立刻解决所有事，先喘口气吧 🫂","焦虑的时候，专注当下每一秒 🕰️"],
            "calm": ["心里的声音终于小下去了。值得为此停一分钟 ✨","这份平静真的很珍贵，好好感受吧 🌊","慢慢来，生活本来就是这样的 🍵"]
        }
...
def ai_privacy_detect():
    try:
        ...
        prompt = f"""
        你是隐私保护助手，识别并打码所有隐私信息：
        1. 手机号、身份证、邮箱、地址、微信号、QQ号
        2. 规则：手机号前3后4，地址保留市区，账号前后各1位
        3. 返回JSON：{{"masked_text": "", "has_sensitive": true/false}}
        文本：{text}
        """

        headers = {"Authorization": f"Bearer {DASHSCOPE_API_KEY}","Content-Type": "application/json"}
        payload = {"model": "qwen-turbo","input": {"prompt": prompt},"parameters": {"response_format": "json"}}
        response = requests.post(DASHSCOPE_URL, headers=headers, json=payload)
        response.raise_for_status()
        parsed = json.loads(response.json()["output"]["text"])
        return jsonify({
